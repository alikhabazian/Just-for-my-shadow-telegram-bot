...
PREORDER_STATE, PREORDER_CHOOSE, PREORDER_CONFIRMED_STATE, PREORDER_CANCEL_STATE = range(
    4)
...
async def start(update: Update, context: CallbackContext) -> int:
    ...
    try:
        user = db_client.users.find_one({'user_id': update.effective_chat.id})
        if user:
            print("user is here")
            return
    except Exception as e:
...
async def acceptHandeling(update: Update, context: CallbackContext) -> int:
    ...
    db_client.payments.update_one({'transaction_id': credentials['transaction_id']}, {
                                  "$set": {"verified": True}})
...
async def rejectHandeling(update: Update, context: CallbackContext) -> int:
    ...
    print(db_client.payments.update_one(
        {'transaction_id': credentials['transaction_id']}, {"$set": {"rejected": True}}))
...
def get_user_credentials(effective_message):
    ...
    if effective_message.text:
        payment_receipt = effective_message.text.split(
            '-------------------------')[1]
